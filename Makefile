...
pkgdatadir = $(datadir)/kryptoniexcore
pkgincludedir = $(includedir)/kryptoniexcore
pkglibdir = $(libdir)/kryptoniexcore
pkglibexecdir = $(libexecdir)/kryptoniexcore
...
CONFIG_HEADER = $(top_builddir)/src/config/kryptoniex-config.h
CONFIG_CLEAN_FILES = libkryptoniexconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libkryptoniexconsensus.pc.in \
	...
	$(top_srcdir)/src/config/kryptoniex-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/KRYPTONIEX/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/KRYPTONIEX/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/KRYPTONIEX/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/KRYPTONIEX/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = kryptoniex-cli
BITCOIN_DAEMON_NAME = kryptoniexd
BITCOIN_GUI_NAME = kryptoniex-qt
BITCOIN_TX_NAME = kryptoniex-tx
...
COPYRIGHT_HOLDERS_FINAL = The KRYPTONIEX Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = KRYPTONIEX Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/KRYPTONIEX/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = kryptoniexcore
PACKAGE_BUGREPORT = https://github.com/kryptoniexpay/kryptoniex/issues
PACKAGE_NAME = KRYPTONIEX Core
PACKAGE_STRING = KRYPTONIEX Core 1.0.0
PACKAGE_TARNAME = kryptoniexcore
PACKAGE_URL = https://kryptoniex.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/KRYPTONIEX
abs_srcdir = /tmp/KRYPTONIEX
abs_top_builddir = /tmp/KRYPTONIEX
abs_top_srcdir = /tmp/KRYPTONIEX
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/KRYPTONIEX/build-aux/install-sh
...
pkgconfig_DATA = libkryptoniexconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/kryptoniex-cli.bash-completion \
	       $(top_srcdir)/contrib/kryptoniex-tx.bash-completion \
	       $(top_srcdir)/contrib/kryptoniexd.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_kryptoniex_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_kryptoniex_coverage.info test_kryptoniex.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/KRYPTONIEX-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/kryptoniex-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/kryptoniex-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/kryptoniex-config.h
$(top_srcdir)/src/config/kryptoniex-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/kryptoniex-config.h src/config/stamp-h1
libkryptoniexconsensus.pc: $(top_builddir)/config.status $(srcdir)/libkryptoniexconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/KRYPTONIEX-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/KRYPTONIEX-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/KRYPTONIEX-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_kryptoniex.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_kryptoniex -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_kryptoniex_filtered.info: test_kryptoniex.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_kryptoniex_filtered.info
...
#test_kryptoniex_coverage.info: baseline_filtered_combined.info test_kryptoniex_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_kryptoniex_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_kryptoniex_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_kryptoniex_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_kryptoniex.coverage/.dirstamp:  test_kryptoniex_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_kryptoniex.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_kryptoniex.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
