...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in kryptoniex.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("kryptoniex.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* KRYPTONIEX features */
    { "kryptoniex",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "kryptoniex",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "kryptoniex",               "gobject",                &gobject,                true,  {} },
    { "kryptoniex",               "voteraw",                &voteraw,                true,  {} },
