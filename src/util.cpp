...
const char * const BITCOIN_CONF_FILENAME = "kryptoniex.conf";
const char * const BITCOIN_PID_FILENAME = "kryptoniexd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "kryptoniex" is a composite category enabling all KRYPTONIEX-related debug output
                if(ptrCategory->count(std::string("kryptoniex"))) {
...
#else
    const char* pszModule = "kryptoniex";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KRYPTONIEX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KRYPTONIEX
    // Mac: ~/Library/Application Support/KRYPTONIEX
    // Unix: ~/.kryptoniex
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KRYPTONIEX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KRYPTONIEX";
#else
    // Unix
    return pathRet / ".kryptoniex";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty kryptoniex.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override kryptoniex.conf
